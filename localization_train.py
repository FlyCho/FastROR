...
tf.app.flags.DEFINE_integer('train_stage', 0, '0-train detection only; 1-train recognition only; 2-train end-to-end; 3-train end-to-end absolutely')
tf.app.flags.DEFINE_string('training_data_dir', default='D:/FOTS/FOTS_TF-dev/datasets/PS1_PS2_vertical_dataset/train/JPEGImages/', help='training images dir')
tf.app.flags.DEFINE_string('training_gt_data_dir', default='D:/FOTS/FOTS_TF-dev/datasets/PS1_PS2_vertical_dataset/train/Annotations/', help='training gt dir')
...
def compute_loss(f_score, f_geometry, input_score_maps, input_geo_maps, input_training_masks, lamda=0.01):
    ...
    return detection_loss, model_loss
...
def main(argv=None):
    ...
    # build the graph
    f_score, f_geometry = build_graph(input_images, input_transform_matrix, input_box_masks, input_box_widths, input_seq_len)
    ...
    # calculate loss
    d_loss, model_loss = compute_loss(f_score, f_geometry, input_score_maps, input_geo_maps, input_training_masks)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            dl, tl, _ = sess.run([d_loss, total_loss, train_op], feed_dict=inp_dict)
            if np.isnan(tl):
            ...
            if step % 10 == 0:
                ...
                start = time.time()
                print('Step {:06d}, detect_loss {:.4f}, total loss {:.4f}, {:.2f} seconds/step, {:.2f} examples/second'.format(
                    step, dl, tl, avg_time_per_step, avg_examples_per_second))
            ...
            if step % FLAGS.save_summary_steps == 0:
                ...
                _, tl, summary_str = sess.run([train_op, total_loss, summary_op], feed_dict={input_images: data[0],
                                                                                             ...
                                                                                             input_training_masks: data[4]})
                """
                dl, tl, _, summary_str = sess.run([d_loss, total_loss, train_op, summary_op], feed_dict=inp_dict)
